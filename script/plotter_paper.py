...
if plot2:
    ...
    im = ax_texc.imshow(np.log10(T_spin / T_CMB_z), cmap=matplotlib.cm.inferno, aspect="auto", \
                        norm=matplotlib.colors.PowerNorm(gamma=0.2), interpolation="nearest",\
                        extent=[-5-8/2/n_points,3+8/2/n_points,-19-8/2/n_points,-12+8/2/n_points])

    # Create colorbar

    cax = plt.axes([0.06, 0.15, 0.014, 0.8])

    ticks = [0.1,0.4,1.0,2.0]
