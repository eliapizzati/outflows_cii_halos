...
if plot2:
    ...
    kwargs = dict(
            bins=50, smooth=0.9, labels=labels, label_kwargs=dict(labelpad=8),
    ...
    fig.text(0.705, 0.825, "corner plot for", ha='center', size=19)
...
if plot4:
    ...
    for data, beta, likelihood in zip(datas, betas, logprobs):
        ...
        sigma_betas_up.append(np.percentile(beta, 0.84))
        sigma_betas_down.append(np.percentile(beta, 0.16))
    ...
    ax_vc.scatter(mstars, beta_means,\
               marker='o', color = cmap_rend_col((sfrs-SFR_min)/(SFR_max-SFR_min)),\
               s=500*(1./abs(likelihood_means)) )
    

    ax_vc.errorbar(mstars, beta_means, \
                     [sigma_betas_down,sigma_betas_up],\
    ...
    cax_vc = plt.axes([0.07, 0.16, 0.018,0.8])
    ...
    ax_sfr.scatter(sfrs, beta_means,\
               marker='o', color = cmap_rend_col((vcs-v_c_min)/(v_c_max-v_c_min)),\
               s=500*(1./abs(likelihood_means)) )
    

    ax_sfr.errorbar(sfrs, beta_means, \
                     [sigma_betas_down,sigma_betas_up],\
    ...
    cax_sfr = plt.axes([0.9, 0.16, 0.018,0.8])
    ...
    cb.set_label(r"$v_c$ [km s$^{-1}$]", rotation=90., labelpad=0)
    ...
    ax_sfr.fill_between(xaxis, power(xaxis, a-sigma_a ,b-sigma_b), power(xaxis, a+sigma_a ,b+sigma_b), color="C0", alpha=0.05)
