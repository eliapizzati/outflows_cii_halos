...
names = ["DEIMOS_COSMOS_351640", "DEIMOS_COSMOS_396844", "DEIMOS_COSMOS_416105", "DEIMOS_COSMOS_488399", \
         ...
         "DEIMOS_COSMOS_683613", "DEIMOS_COSMOS_709575", "DEIMOS_COSMOS_733857", "DEIMOS_COSMOS_834764",
         # "DEIMOS_COSMOS_848185",\
...
names_CII_halo = ["DEIMOS_COSMOS_396844", #"DEIMOS_COSMOS_494057",
                  "DEIMOS_COSMOS_630594", "DEIMOS_COSMOS_683613", \
...
names_CII_halo_short = ["DC_396844", #"DC_494057",
                         "DC_630594", "DC_683613", "DC_880016", \
...
if __name__ == "__main__":
    ...
    ax_tot.set_xlabel("b [kpc]")
    ax_tot.set_ylabel("data [mJy/arcsec^2]")
...
if __name__ == "__main__":
    if latex == True:
        print("\hline")
        print("Name  ", "&", "redshift   ", "&", "\mathrm{SFR} [$\msun\,\mathrm{yr}^{-1}$]  ", "&",
...
for name, name_short, SFR_IR, SFR_IR_err_down, SFR_IR_err_up in \
    ...
    SFR_lim_down = 10 ** log_SFR_lim_down

    SFR_err_up = SFR_lim_up - SFR
    SFR_err_down = SFR - SFR_lim_down
    ...
    params_obs = dict([("name", name),
                       ...
                       ("SFR", SFR),
                       ("SFR_err_up", SFR_err_up),
                       ("SFR_err_down", SFR_err_down),
                       ("log_SFR", log_SFR),
                       ("log_SFR_err_up", log_SFR_lim_up - log_SFR),
                       ("log_SFR_err_down", log_SFR - log_SFR_lim_down),
                       ("v_c", v_c / 1e5),
                       ("v_c_err_up", (v_c_lim_up - v_c) / 1e5),
                       ("v_c_err_down", (v_c - v_c_lim_down) / 1e5),
                       ("log_v_c", np.log10(v_c / 1e5)),
                       ("log_v_c_err_up", np.log10(v_c_lim_up / 1e5) - np.log10(v_c / 1e5) ),
                       ("log_v_c_err_down", np.log10(v_c / 1e5) - np.log10(v_c_lim_down / 1e5) ),
    ...
    if __name__ == "__main__":
        ...
        #ax_star.errorbar(10**log_M_star, SFR, [[SFR_err_down],[SFR_err_up]],
        #                  [ [10 ** log_M_star - 10 ** log_M_star_lim_down],[ 10 ** log_M_star_lim_up - 10 ** log_M_star]],
        #                  label=f"{name_short}"
        #                  )

        ax_star.errorbar(params_obs["log_v_c"], params_obs["log_SFR"], \
                         [[params_obs["log_SFR_err_down"]], [params_obs["log_SFR_err_up"]]],
                         [[params_obs["log_v_c_err_down"]],[params_obs["log_v_c_err_up"]]],
                         label=f"{name_short}"
                         )

        if latex == True:
            print(
                "{0:}    &    {1:.2f} \pm{14:}   &    {2:.0f}^+{3:.0f}_{4:.0f}    &   {5:.1f}^+{6:.1f}_{7:.1f}   &   {8:.1f}^+{9:.1f}_{10:.1f}    &   {11:.0f}^+{12:.0f}_{13:.0f}". \
                format(name_short, redshift, SFR, SFR_err_up, SFR_err_down, \
                       10 ** log_M_star / 1e9, (10 ** log_M_star_lim_up - 10 ** log_M_star) / 1e9,
                       (10 ** log_M_star - 10 ** log_M_star_lim_down) / 1e9, \
            ...
            print(name_short, "\t", round(SFR, 1), "\p", round(SFR_err_up, 1), "\m",
                  round(SFR_err_down, 1), \
                  "\t", round(10 ** log_M_star / 1e10, 3), "\t", round(M_halo / 1e11, 3), "\p",
                  round(M_halo_lim_up / 1e11, 3), "\m", round(M_halo_lim_down / 1e11, 3), \
                  "\t", round(redshift, 2), "\t", round(v_c / 1e5, 1), "\p", round(v_c_lim_up / 1e5), "\m",
                  round(v_c_lim_down / 1e5), halo_class)
...
if __name__ == "__main__":
    ...
    ax_star.set_xlabel("v_c")
    ax_star.set_ylabel("SFR")
    ax_star.legend()
    plt.show()
...
observational_data_fuji = obs_data(x_data=fuji_x, y_data=fuji_y, x_beam=beam_x, y_beam=beam_y, err=err,
                                   params_obs=params_obs)
